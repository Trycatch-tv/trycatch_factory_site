...
export const useSubmitHandler = () => {
    const handleSubmit = async (event: any) => {
      ...
      const dataPost = Object.fromEntries(formData.entries());
      ...
      submitButton.innerHTML = loadingComponent;
  
      try {
        const response = await sendData(dataPost);
        if (response.status === 201) {
          alertContainer.innerHTML = successComponent;
          form.reset();
        }
      } catch (err: any) {
        alertContainer.innerHTML = errorComponent(err.error.details);
        console.error(err);
      } finally {
